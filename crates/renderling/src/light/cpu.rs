...
pub struct ShadowMap {
    ...
    desc: SlabAllocator<wgpu::Buffer>,
...
impl ShadowMap {
    ...
    pub fn new(ctx: &crate::Context, light_transform: Mat4, size: wgpu::Extent3d) -> Self {
    ...
    fn get_bindgroup(
        ...
        let (desc_buffer, desc_buffer_was_updated) = self.desc.get_updated_buffer_and_check((
            device,
            queue,
            Self::LABEL,
        ...
        if let Some(bg) = guard.as_ref() {
            ...
            log::trace!("creating shadow-mapping desc bindgroup");
            let bg: Arc<_> = device
...
mod test {
    ...
    fn shadow_mapping_sanity() {
        ...
        shadows.update(&ctx, &stage.get_buffer().unwrap(), doc.renderlets_iter());
