...
static SKYBOX_CUBEMAP_VERTEX: Mutex<Option<Arc<wgpu::ShaderModule>>> = Mutex::new(None);
fn get_module(device: &wgpu::Device) -> Arc<wgpu::ShaderModule> {
    let mut guard = SKYBOX_CUBEMAP_VERTEX.lock().unwrap();
    if let Some(module) = guard.as_ref() {
        ...
        log::debug!(
            ...
            stringify!(skybox_cubemap_vertex)
        );
        let module = Arc::new(
...
pub fn linkage(device: &wgpu::Device) -> ShaderLinkage {
    ShaderLinkage {
        ...
        entry_point: "skybox::skybox_cubemap_vertex",
    }
}
