...
static GENERATE_MIPMAP_VERTEX: Mutex<Option<Arc<wgpu::ShaderModule>>> = Mutex::new(None);
fn get_module(device: &wgpu::Device) -> Arc<wgpu::ShaderModule> {
    let mut guard = GENERATE_MIPMAP_VERTEX.lock().unwrap();
    if let Some(module) = guard.as_ref() {
        ...
        log::debug!(
            ...
            stringify!(generate_mipmap_vertex)
        );
...
pub fn linkage(device: &wgpu::Device) -> ShaderLinkage {
    ShaderLinkage {
        ...
        entry_point: "convolution::generate_mipmap_vertex",
    }
}
