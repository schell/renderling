...
use glam::Vec3;
use renderling::shader::{
    gltf::{GltfMesh, GltfNode, GltfPrimitive},
    pbr::Material,
    stage::{RenderUnit, Transform, Vertex},
};
...
pub struct Rectangle {
    aabb: AABB,
    primitive: Id<GltfPrimitive>,
    material: Id<Material>,
    render_unit: Id<RenderUnit>,
    updates: Stage,
}

impl Rectangle {
    // Create a new rectangle.
    pub fn new(gpui: &Gpui) -> Self {
        let mut stage = gpui.stage.clone();
        let material = Id::NONE;
        let primitive = stage.new_primitive(Self::vertices(), None, material);
        ...
        let render_unit = stage.draw_unit(&RenderUnit {
            node_path,
            camera: gpui.camera,
            vertex_count: 6,
            ..Default::default()
    ...
    pub fn set_origin(&mut self, origin: impl Into<Vec2>) {
        ...
        if self.aabb.min != origin {
            ...
            self.stage.write(
                self.render_unit
                    + RenderUnit::offset_of_transform()
                    + Transform::offset_of_translation(),
                &Vec3::new(self.aabb.min.x, self.aabb.min.y, 0.0),
            );
        }
    }
    ...
    fn vertices() -> [Vertex; 6] {
        let tl = Vertex::default().with_position(Vec3::ZERO);
        let tr = Vertex::default().with_position(Vec3::new(1.0, 0.0, 0.0));
        let bl = Vertex::default().with_position(Vec3::new(0.0, 1.0, 0.0));
        let br = Vertex::default().with_position(Vec3::new(1.0, 1.0, 0.0));
        [tl, bl, br, tl, br, tr]
    }

    //fn allocate(&mut self, gpui: &mut Gpui) -> {
...
//        Text {
//            section:
// OwnedSection::default().add_text(OwnedText::new("").with_scale(12.0)),
//            cache,
//            aabb: AABB::default(),
//            updated: true,
//            draw_object: None,
...
//    ///
//    /// This will return `AABB::default` until [`Rectangle::layout`] is
// called.    pub fn aabb(&self) -> AABB {
//        self.aabb
//    }
//}
//
...
//                self.aabb.max = self.aabb.min + Vec2::new(rect.width(),
// rect.height());            }
//        }
//        self.aabb
//    }
...
//        }
//
//        let (may_vertices, may_texture) = self.cache.get_updated(device,
// queue);        if let Some(verts) = may_vertices {
...
//            background: Rectangle::new().with_color(Vec4::new(0.0, 0.0, 0.0,
// 0.5)),            text: Text::new(font)
//                .with_text("Button")
//                .with_scale(16.0)
...
//                let position = Vec2::new(position.x as f32, position.y as
// f32);                if self.aabb.contains(position) {
//                    if is_down {
//                        self.set_down();
//                    } else {
//                        self.set_over();
...
//            (ButtonState::Normal, ButtonState::Over) =>
// Some(ButtonEvent::Over),            (ButtonState::Normal, ButtonState::Down)
// => None,            (ButtonState::Over, ButtonState::Normal) =>
// Some(ButtonEvent::Out),            (ButtonState::Over, ButtonState::Over) =>
// None,            (ButtonState::Over, ButtonState::Down) =>
// Some(ButtonEvent::Down),            (ButtonState::Down, ButtonState::Normal)
// => None,            (ButtonState::Down, ButtonState::Over) =>
// Some(ButtonEvent::Up),            (ButtonState::Down, ButtonState::Down) =>
// None,        }
//    }
//}
//
...
//    }
//
//    pub fn set_selections(&mut self, selections: impl IntoIterator<Item =
// (String, T)>) {        self.selections = selections.into_iter().collect();
//        self.buttons = self
//            .selections
//            .iter()
//            .map(|(name, _)| {
...
//    }
//
//    pub fn with_selections(mut self, selections: impl IntoIterator<Item =
// (String, T)>) -> Self {        self.set_selections(selections);
//        self
//    }
//
